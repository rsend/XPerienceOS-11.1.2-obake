...
EOF
    T=$(gettop)
    for i in `cat $T/build/envsetup.sh $T/vendor/XPe/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      echo "$i"
    done | column
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/XPe/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/XPe/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function check_product()
{
    ...
    elif (echo -n $1 | grep -q -e "^xpe_") ; then
        # Fall back to xpe <product>
        XPE_BUILD=$(echo -n $1 | sed -e 's/^xpe_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $XPE_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        XPE_BUILD=
...
function lunch()
{
    ...
    # This release tree is self-contained. Never fetch products or manifests
    # from an upstream repository as a side effect of lunch.
    if ! check_product "$product"; then
        echo "** Product '$product' is not present in this source tree" >&2
        return 1
    fi
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/XPe/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
. $ANDROID_BUILD_TOP/vendor/XPe/build/envsetup.sh
