...
# The selected SU6-7.3 lib-imsdpl has a fixed /vendor/lib/lib-imsqimf.so
# lookup, while lib-imsrcs uses its normal DT_NEEDED lookup.  Both paths must
# identify the same QIMF object: loading the older vendor blob beside the
# selected radio stack creates two independent registration-manager
# singletons in one process.
IMS_QIMF_SELECTED := $(TARGET_OUT)/lib/radio-su6-73/lib-imsqimf.so
IMS_QIMF_COMPAT_LINK := $(TARGET_OUT_VENDOR)/lib/lib-imsqimf.so
$(IMS_QIMF_COMPAT_LINK): $(IMS_QIMF_SELECTED) FORCE
	@echo "IMS QIMF compatibility link: $@"
	@mkdir -p $(dir $@)
	@rm -f $@
	$(hide) ln -sf /system/lib/radio-su6-73/lib-imsqimf.so $@

ALL_DEFAULT_INSTALLED_MODULES += $(IMS_QIMF_COMPAT_LINK)
