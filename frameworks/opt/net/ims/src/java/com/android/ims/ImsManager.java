...
public class ImsManager {
    ...
    public static void updateImsServiceConfig(Context context, int phoneId, boolean force) {
        ...
        // CarrierConfig returns platform defaults while the subscription for this
        // phone is not active.  The default VoLTE value is false, so applying it
        // here turns IMS off and marks the configuration complete before the
        // carrier-specific bundle is available.  Leave mConfigUpdated false and
        // let the subsequent carrier-config-changed event retry with the active
        // subscription instead.
        final int[] subIds = SubscriptionManager.getSubId(phoneId);
        final SubscriptionManager subscriptionManager = SubscriptionManager.from(context);
        if (subIds == null || subIds.length == 0 || subscriptionManager == null
                || !subscriptionManager.isActiveSubId(subIds[0])) {
            log("updateImsServiceConfig: active subscription not ready for phoneId="
                    + phoneId + ", skipping");
            return;
        }
