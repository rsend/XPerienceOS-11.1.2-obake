...
public class GsmCdmaPhone extends Phone {
    ...
    protected void phoneObjectUpdater(int newVoiceRadioTech) {
        ...
        // Some legacy multimode RILs report a dormant CDMA voice RAT before a GSM SIM has
        // registered. Do not let that stale report replace a GSM phone when the configured
        // network mode explicitly excludes CDMA.
        int preferredNetworkMode = PhoneFactory.calculatePreferredNetworkType(
                mContext, getSubId());
        if (mCi instanceof RIL
                && ((RIL) mCi).needsOldRilFeature("ignorecdmavoicetechongsm")
                && isPhoneTypeGsm()
                && preferredNetworkMode == RILConstants.NETWORK_MODE_LTE_GSM_WCDMA
                && ServiceState.isCdma(newVoiceRadioTech)) {
            logd("Ignoring CDMA voice RAT " + newVoiceRadioTech
                    + " in LTE/GSM/WCDMA mode");
            return;
        }
