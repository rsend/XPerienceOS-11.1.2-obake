...
public abstract class Phone extends Handler implements PhoneInternalInterface {
    ...
    private HomeNetworkSelection mHomeNetworkSelection;

    private HomeNetworkSelection homeNetworkSelection() {
        if (mHomeNetworkSelection == null) {
            mHomeNetworkSelection = new HomeNetworkSelection(this, mCi);
        }
        return mHomeNetworkSelection;
    }

    void updateHomeNetworkSelection(ServiceState state) {
        if (mHomeNetworkSelection != null
                || SystemProperties.getBoolean(HomeNetworkSelection.PROPERTY, false)) {
            homeNetworkSelection().onServiceState(state);
        }
    }
    ...
    protected void notifyPreciseCallStateChangedP() {
        ...
        if (tracker != null) updateHomeNetworkSelection(tracker.mSS);
    }
    ...
    public void selectNetworkManually(OperatorInfo network, boolean persistSelection,
        ...
        if ((mHomeNetworkSelection != null
                || SystemProperties.getBoolean(HomeNetworkSelection.PROPERTY, false))
                && homeNetworkSelection().selectIfHome(network.getOperatorNumeric(), response)) {
            return;
        }
        // wrap the response message in our own message along with
        // the operator's id.
     ...
     */
    boolean clearSavedNetworkSelection() {
        // open the shared preferences and search with our key.
        boolean committed = PreferenceManager.getDefaultSharedPreferences(getContext()).edit().
        ...
        if (!committed) Rlog.e(LOG_TAG, "Failed to clear saved network selection");
        return committed;
    }
    ...
    */
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        if (mHomeNetworkSelection != null) mHomeNetworkSelection.dump(pw);
