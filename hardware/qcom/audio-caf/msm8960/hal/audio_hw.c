...
static void get_usecase_mixer_path(struct audio_usecase *usecase,
                                  snd_device_t snd_device, char *mixer_path)
{
    strlcpy(mixer_path, use_case_table[usecase->id], MIXER_PATH_MAX_LENGTH);
#ifdef MSM8960_LEGACY_VOICE
    /* The TFA9890 uses MI2S for calls too. Do not add a device-wide backend
     * suffix: media streams sharing voice-speaker must keep their old paths. */
    if (usecase->type == VOICE_CALL && snd_device == SND_DEVICE_OUT_VOICE_SPEAKER) {
...
#endif
    get_usecase_mixer_path(usecase, snd_device, mixer_path);
    ALOGV("%s: apply mixer and update path: %s", __func__, mixer_path);
    int ret = audio_route_apply_and_update_path(adev->audio_route, mixer_path);
    if (ret < 0) {
        ALOGE("%s: route=%s usecase=%d failed: %d", __func__,
              mixer_path, usecase->id, ret);
        return ret;
    }
                        ...
                        struct audio_usecase *usecase)
{
    ...
    int ret = audio_route_reset_and_update_path(adev->audio_route, mixer_path);
    if (ret < 0) {
        ALOGE("%s: reset route=%s usecase=%d failed: %d", __func__,
              mixer_path, usecase->id, ret);
        return ret;
    }
    ALOGV("%s: exit", __func__);
...
int select_devices(struct audio_device *adev, audio_usecase_t uc_id)
{
    ...
    status = enable_audio_route(adev, usecase);
    if (status < 0)
        return status;
