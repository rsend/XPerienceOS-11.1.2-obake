...
void *platform_init(struct audio_device *adev)
{
    ...
    if (my_data->acdb_handle == NULL) {
        ALOGE("%s: DLOPEN failed for %s: %s", __func__, LIB_ACDB_LOADER, dlerror());
    } else {
        ...
        if (my_data->acdb_init == NULL)
            ...
            int acdb_status = my_data->acdb_init();
            if (acdb_status < 0) {
                ALOGE("%s: ACDB initialization failed status=%d", __func__, acdb_status);
                /* Do not let voice startup claim calibration was submitted
                 * after a failed loader initialization. */
                my_data->acdb_send_voice_cal = NULL;
            }
        }
    }
...
#ifdef MSM8960_LEGACY_VOICE
enum legacy_voice_control {
    LEGACY_VOICE_VOLUME,
    LEGACY_VOICE_TX_MUTE,
    LEGACY_VOICE_RX_MUTE,
};

static int set_legacy_voice_control(struct audio_device *adev,
                                    enum legacy_voice_control control, int value)
{
    /* Unlike newer kernels' all-session controls, these are scalar and
     * session-specific. Set inactive sessions too so the next PCM prepare
     * cannot start with stale volume or a stale microphone mute setting. */
    static const char * const names[][3] = {
        {"Voice Rx Volume", "VoLTE Rx Volume", "Voice2 Rx Volume"},
        {"Voice Tx Mute", "VoLTE Tx Mute", "Voice2 Tx Mute"},
        {"Voice Rx Device Mute", "VoLTE Rx Device Mute", "Voice2 Rx Device Mute"},
    };
    int first_error = 0;
    size_t i;

    for (i = 0; i < 3; ++i) {
        const char *name = names[control][i];
        struct mixer_ctl *ctl = mixer_get_ctl_by_name(adev->mixer, name);
        int ret;
        if (!ctl || mixer_ctl_get_num_values(ctl) != 1) {
            ret = -EINVAL;
            ALOGE("%s: missing/non-scalar control=%s", __func__, name);
        } else {
            if (control == LEGACY_VOICE_VOLUME) {
                const int min = mixer_ctl_get_range_min(ctl);
                const int max = mixer_ctl_get_range_max(ctl);
                /* SU6 libalsa-intf mixer_ctl_set takes an attenuation
                 * percentage, then rounds into this control's range (0..7).
                 * Android's UI step count is not the DSP table's range. */
                if (value < 0 || value > 100 || min != 0 || max != 7) {
                    ALOGE("%s: invalid volume ABI control=%s range=%d..%d percent=%d",
                          __func__, name, min, max, value);
                    ret = -EINVAL;
                } else {
                    const int index = (value * (max - min) + 50) / 100 + min;
                    ret = mixer_ctl_set_value(ctl, 0, index);
                }
            } else {
                ret = mixer_ctl_set_value(ctl, 0, value);
            }
            if (ret < 0)
                ALOGE("%s: control=%s value=%d error=%d", __func__, name, value, ret);
        }
...
#ifdef MSM8960_LEGACY_VOICE
    return set_legacy_voice_control(adev, LEGACY_VOICE_VOLUME, volume);
#endif
...
#ifdef MSM8960_LEGACY_VOICE
    return set_legacy_voice_control(adev, LEGACY_VOICE_TX_MUTE, state);
#endif
...
#ifdef MSM8960_LEGACY_VOICE
    if (strcmp(dir, "rx") == 0)
        return set_legacy_voice_control(adev, LEGACY_VOICE_RX_MUTE, state);
    if (strcmp(dir, "tx") == 0)
        return set_legacy_voice_control(adev, LEGACY_VOICE_TX_MUTE, state);
    return -EINVAL;
#endif
