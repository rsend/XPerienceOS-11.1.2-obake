...
int voice_start_usecase(struct audio_device *adev, audio_usecase_t usecase_id)
{
    ...
    ret = select_devices(adev, usecase_id);
    if (ret < 0) {
        ALOGE("%s: route setup failed usecase=%d vsid=%x error=%d",
              __func__, usecase_id, session->vsid, ret);
    ...
    /* The old kernel stores controls separately for CS, VoLTE and Voice2.
     * Restore cached policy before opening PCMs (prepare can start voice). */
    ret = voice_set_volume(adev, adev->voice.volume);
    if (ret == 0)
        ret = voice_set_mic_mute(adev, adev->voice.mic_mute);
    if (ret < 0) {
        ALOGE("%s: volume/mute setup failed usecase=%d vsid=%x error=%d",
              __func__, usecase_id, session->vsid, ret);
        goto error_start_voice;
    }
#endif
    ...
    if (!session->pcm_rx || !pcm_is_ready(session->pcm_rx)) {
        ALOGE("%s: RX card=%d pcm=%d vsid=%x: %s", __func__,
              adev->snd_card, pcm_dev_rx_id, session->vsid,
              session->pcm_rx ? pcm_get_error(session->pcm_rx) : "null PCM");
        ret = -EIO;
        goto error_start_voice;
    }
    ...
    if (!session->pcm_tx || !pcm_is_ready(session->pcm_tx)) {
        ALOGE("%s: TX card=%d pcm=%d vsid=%x: %s", __func__,
              adev->snd_card, pcm_dev_tx_id, session->vsid,
              session->pcm_tx ? pcm_get_error(session->pcm_tx) : "null PCM");
        ret = -EIO;
        goto error_start_voice;
    }
    ret = pcm_start(session->pcm_rx);
    if (ret < 0) {
        ALOGE("%s: RX start pcm=%d vsid=%x: %s", __func__,
              pcm_dev_rx_id, session->vsid, pcm_get_error(session->pcm_rx));
        goto error_start_voice;
    }
    ret = pcm_start(session->pcm_tx);
    if (ret < 0) {
        ALOGE("%s: TX start pcm=%d vsid=%x: %s", __func__,
              pcm_dev_tx_id, session->vsid, pcm_get_error(session->pcm_tx));
        goto error_start_voice;
    }

#ifndef MSM8960_LEGACY_VOICE
...
#ifdef MSM8960_LEGACY_VOICE
    if (!isfinite(volume)) {
        ALOGE("%s: non-finite voice volume", __func__);
        return -EINVAL;
    }
#endif
