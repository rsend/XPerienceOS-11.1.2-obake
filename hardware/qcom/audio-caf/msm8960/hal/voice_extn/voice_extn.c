...
static bool is_valid_vsid(uint32_t vsid)
{
    ...
    /* No QCHAT/WLAN PCM frontends exist on this integrated-modem kernel. */
    return vsid == VOICE_VSID || vsid == VOICE2_VSID || vsid == VOLTE_VSID;
#else
    if (vsid == VOICE_VSID ||
        vsid == VOICE2_VSID ||
        vsid == VOLTE_VSID ||
...
#else
                ret = -ENOSYS;
                ALOGE("%s: local hold ioctl unavailable vsid=%x", __func__, session->vsid);
#endif
                break;
