...
{
	int volume = voc_get_rx_vol_index(voc_get_session_id(VOICE_SESSION_NAME));
	if (volume < 0)
		return volume;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: volume: %d\n", __func__, volume);
	return voc_set_rx_vol_index(voc_get_session_id(VOICE_SESSION_NAME),
						RX_PATH, volume);
}
...
static int msm_volte_volume_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int volume = voc_get_rx_vol_index(voc_get_session_id(VOLTE_SESSION_NAME));
	if (volume < 0)
		return volume;
	ucontrol->value.integer.value[0] = volume;
	return 0;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	int volume = ucontrol->value.integer.value[0];
	pr_debug("%s: volume: %d\n", __func__, volume);
	return voc_set_rx_vol_index(voc_get_session_id(VOLTE_SESSION_NAME),
						RX_PATH, volume);
}
...
static int msm_voice2_volume_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int volume = voc_get_rx_vol_index(voc_get_session_id(VOICE2_SESSION_NAME));
	if (volume < 0)
		return volume;
	ucontrol->value.integer.value[0] = volume;
	return 0;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	return voc_set_rx_vol_index(voc_get_session_id(VOICE2_SESSION_NAME),
			     RX_PATH, volume);
}
