...
static int voice_send_vol_index_cmd(struct voice_data *v)
{
	...
	memset(&cvp_vol_cmd, 0, sizeof(cvp_vol_cmd));
	if (v == NULL) {
		pr_err("%s: v is NULL\n", __func__);
		return -EINVAL;
	}
	...
	if (++v->cvp_volume_token == 0)
		++v->cvp_volume_token;
	v->cvp_volume_ack_token = 0;
	v->cvp_volume_status = ~0U;
	cvp_vol_cmd.hdr.token = v->cvp_volume_token;
	...
	if (ret < 0) {
		pr_err("%s: session=%x index=%u send failed=%d\n",
			__func__, v->session_id, v->dev_rx.volume, ret);
		return ret;
	}
	ret = wait_event_timeout(v->cvp_wait,
				 (ACCESS_ONCE(v->cvp_volume_ack_token) == v->cvp_volume_token),
				 msecs_to_jiffies(TIMEOUT_MS));
	if (!ret) {
		pr_err("%s: session=%x index=%u token=%u timeout\n",
			__func__, v->session_id, v->dev_rx.volume, v->cvp_volume_token);
		return -ETIMEDOUT;
	}
	smp_rmb();
	if (v->cvp_volume_status != 0)
		return -EREMOTEIO;
	return 0;
}
...
int voc_set_rx_vol_index(uint16_t session_id, uint32_t dir, uint32_t vol_idx)
{
	...
	if (v == NULL || dir != RX_PATH || vol_idx > 7) {
		pr_err("%s: invalid session=%x direction=%u index=%u\n",
			__func__, session_id, dir, vol_idx);
...
/* Requested index, not an assertion that the DSP applied it. */
int voc_get_rx_vol_index(uint16_t session_id)
{
	struct voice_data *v = voice_get_session(session_id);
	int volume;
	if (v == NULL)
...
static int32_t qdsp_cvp_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == APR_BASIC_RSP_RESULT) {
		if (data->payload_size < 2 * sizeof(uint32_t) || !data->payload) {
			pr_err_ratelimited("%s: short/null CVP reply session=%x size=%u\n",
				__func__, v->session_id, data->payload_size);
			return -EINVAL;
		}
		{
			ptr = data->payload;
			...
			if (ptr[1] != 0)
				pr_err_ratelimited("%s: DSP rejected session=%x opcode=%x status=%x token=%u\n",
					__func__, v->session_id, ptr[0], ptr[1], data->token);

			switch (ptr[0]) {
			case VSS_IVOCPROC_CMD_SET_RX_VOLUME_INDEX:
				if (!data->token || data->token != v->cvp_volume_token)
					break;
				v->cvp_volume_status = ptr[1];
				/* Publish status before making the wait condition true. */
				smp_wmb();
				v->cvp_volume_ack_token = data->token;
				wake_up(&v->cvp_wait);
				break;
