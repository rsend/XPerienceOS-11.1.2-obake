...
struct voice_data {
	...
	u32 cvs_state;
	u32 cvp_state;
	/* Volume replies have their own token/status: a CVP NACK is not success,
	 * and a late reply must not complete a subsequent volume request. */
	u32 cvp_volume_token;
	u32 cvp_volume_ack_token;
	u32 cvp_volume_status;
