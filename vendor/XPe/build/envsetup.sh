...
# Jack 1.3-a8 is a historical local TLS endpoint. Scope the compatibility
# exception to its JVM and force the client to use TLS 1.2; do not modify the
# host-wide Java security policy.
JACK_SECURITY_PROPERTIES="$(gettop)/build/host/java.security-jack"
if [ -r "$JACK_SECURITY_PROPERTIES" ]; then
    case "${ANDROID_JACK_VM_ARGS:-}" in
        *-Djava.security.properties=*) ;;
        *) export ANDROID_JACK_VM_ARGS="${ANDROID_JACK_VM_ARGS:+$ANDROID_JACK_VM_ARGS }-Djava.security.properties=$JACK_SECURITY_PROPERTIES" ;;
    esac
    : "${JACK_EXTRA_CURL_OPTIONS:=--tlsv1.2}"
    export JACK_EXTRA_CURL_OPTIONS
fi
unset JACK_SECURITY_PROPERTIES
